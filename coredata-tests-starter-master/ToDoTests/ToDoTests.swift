...
class ToDoTests: XCTestCase {
    ...
    func test_create_todo() {
        ...
        // When adding an item
        let item = manager.insertTodoItem(name: name, completed: complete)
    ...
    func test_fetch_all_todo() {
        ...
        let item1 = manager.insertTodoItem(name: name1, completed: completion1)
        let item2 = manager.insertTodoItem(name: name2, completed: completion1)
        manager.save()
        
        // Assert the managers total count is equal to the mock items total count
        XCTAssertEqual(manager.fetchAllItems().count, itemsTotalCount())
    ...
    func test_remove_todo() {
        ...
        let allItems = manager.fetchAllItems()
        let firstObjectId = allItems.first?.objectID
        manager.removeItem(objectID: firstObjectId!)
    ...
    func createMockItems() {
        ...
        let item1 = insertTodoItem(name: "Rinni", completed: false)
        let item2 = insertTodoItem(name: "Sarin", completed: false)
        let item3 = insertTodoItem(name: "Ruhsane", completed: false)
        let item4 = insertTodoItem(name: "Ryan", completed: false)
        let item5 = insertTodoItem(name: "Jackson", completed: false)
